...
def panel(name, color=None):
    ...
    with c1:
        p = st.radio("Price", ["Line", "Bar"], key=f"{name}_p")
    with c2:
        v = st.radio("ΔVolume", ["Line", "Bar"], key=f"{name}_v")
    with c3:
        o = st.radio("ΔOI", ["Line", "Bar"], key=f"{name}_o")
...
if not strikes:
    ...
    def details(prefix, label_emoji):
        ...
        if not all(c in df_r.columns for c in cols):
            return {}
